...
conn_string = "host='localhost' dbname='pepernotendatabase' user='postgres' password='pwd' port='5433'"
        ...
        for status in statuses:
            ...
            try:
                ...
                try:
                    cur2 = conn.cursor()
                    cur2.execute(
                        "INSERT INTO " + tweets_table + "(tweetid, text, latitude, longitude) VALUES(%s, %s, %s, %s)",
                        (tweetid, text, latitude, longitude,))
                    cur2.close()
                ...
                try:
                    ...
                    cur2.execute(
                        "INSERT INTO " + tweets_table + "(tweetid, text, latitude, longitude) VALUES( %s, %s, %s, %s)",
                        (tweetid, text, latitude, longitude, json.dumps(place),))
                    cur2.close()
                except Exception as e:
                    # print("Twitter Data insertion failed! " + str(e))
                    # error_type = sys.exc_info()[0]
