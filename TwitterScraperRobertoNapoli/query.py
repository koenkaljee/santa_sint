...
def query_tweets(query, limit=None):
    ...
    while limit is None or len(tweets) < limit:
        ...
        new_tweets, pos = query_tweets_once(query, limit, len(tweets))
...
def query_tweets_with_pos(query, pos, limit=None):
    tweets = []
    iteration = 1
    ...
    while limit is None or len(tweets) < limit:
        # logging.info("Running iteration no {}, query is {}".format(
        #     iteration, repr(query)))
        new_tweets, pos = query_tweets_once_with_pos(query, pos, limit, len(tweets))
        tweets.extend(new_tweets)
...
def query_tweets_once_with_pos(query, pos, limit=None, num_tweets=0):
